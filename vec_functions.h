...
template<typename T, int N>
vec<T, N> clamp(const vec<T, N>& a, const vec<T, N>& minVal, const vec<T, N>& maxVal){
...
template<typename T, int N>
vec<T, N> faceforward(const vec<T, N>& n, const vec<T, N>& i, const vec<T, N>& nRef){
...
vec<T, N> refract(const vec<T, N>& i, const vec<T, N>& n, float r){
    ...
    if (d < 0.0) { return vec<T, N>(0.0); }
    else { return r * i - (r * dot(n, i) + sqrt(d)) * n; }
}